...
namespace prog {
	...
	void menu(int m, astroid::Astroid &a) {
		...
		case 1: {
			...
			print("enter angle");
			input(t);
			if (t > 180) std::cout << std::setprecision(PRECISION) << "length = " << a.length(degreesToRadians(180 - t)) + a.length() << std::endl;
			else std::cout << std::setprecision(PRECISION) << "length = " << a.length(degreesToRadians(t)) << std::endl;
			break;
		}
		case 2: {
			double t;
